...
namespace com.VR_Robotica.AvatarComponents
{
	...
	public class VRR_SimpleEyeGaze : MonoBehaviour
	{
		...
		public SkinnedMeshRenderer wimpernMesh;
		...
		[SerializeField]private Transform				_focusControlTransform;
		...
		private void Awake()
		{
            Debug.Log("this is the reference: " + FacingForwardReference.forward);
		...
		private void update_EyeRotations()
		{
			...
			Eyes[0].GetComponent<LookatSimple>().target = _focusControlTransform;
			Eyes[1].GetComponent<LookatSimple>().target = _focusControlTransform;

			for (int i = 0; i < Eyes.Length; i++)
			{
				//Eyes[i].LookAt(_focusControlTransform.position);
			}
