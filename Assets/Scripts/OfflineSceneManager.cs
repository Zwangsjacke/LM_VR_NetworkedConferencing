...
public class OfflineSceneManager : MonoBehaviour
{
    ...
    public void ChangeText()
    {
        if(phaseNumber == 1)
        {
            text.text = "Hier kannst du einen Avatar personalisieren. Wenn du fertig bist drücke den roten Knopf.\nWähle deinen Look!";
        }
        if(phaseNumber == 2)
        {
            text.text = "Im Folgenden müsst ihr gemeinsam drei Spiele spielen.\nDazu könnt ihr mit bestimmten Gegenständen interagieren. Diese sind in diesen Raum demonstriert. Nimm sie doch mal in die Hand!\n Drücke den roten Knopf, wenn du bereit für den nächsten Schritt bist.";
        }
        if (phaseNumber == 3)
        {
            text.text = "Für jede Aufgabe habt ihr begrenzt Zeit. Der Wecker zeigt euch, wie lange ihr noch habt. Ist die Zeit abgelaufen, klingelt der Wecker.\nUm zur nächsten Aufgabe zu gelangen, müsst ihr die Wecker abstellen und eure Daumen hochstrecken.\nMit dem Knopf vor dem Radio könnt ihr den Versuchsleiter jederzeit erreichen.\nBist Du bereit die Studie zu starten drücke den roten Knopf";
        }
    }
}
